...
final class SceneInteractionMotionTests: XCTestCase {
    ...
    func testReducedMotionRetainsStaticDoneAndWorkspaceBodySize() async throws {
        ...
        XCTAssertTrue(done.usesNativeMotion)
        XCTAssertFalse(done.nativeMotionLayer.isHidden)
        XCTAssertNil(done.stateDecorationLayer.path, "Done must not retain the old checkmark")
